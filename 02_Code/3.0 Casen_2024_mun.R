...
casen17 <- "casen_2017.dta"

casen <- rio::import(paste0(data_inp, "CASEN/2017/", casen17)) |> 
  clean_names() |> 
  filter(region==13) |>
...
casen24 <- "casen_2024.dta"
casen24_mun <- "casen_2024_provincia_comuna.dta"

casen <- rio::import(paste0(data_inp, "CASEN/2024/", casen24)) |> 
  clean_names() |> 
  filter(region==13) 

casen_mun <- rio::import(paste0(data_inp, "CASEN/2024/", casen24_mun)) |> 
  clean_names() 

casen <- casen |> 
  left_join(casen_mun, by = c("folio", "id_persona")) |>
  dplyr::select(comuna, expc, varstrat, varunit, qautr, pobreza, pobreza_multi) |> 
  drop_na()
...
pobreza_multi_dummy <- as.data.frame(make_dummies(casen$pobreza_multi, prefix = "p_"))
colnames(pobreza_multi_dummy) <- c("no_pob", "pob_multi")
casen <- bind_cols(casen, pobreza_multi_dummy)
...
# Complex survey design for CASEN 2024
# Use varstrat as strata (not estrato)
# Use varunit as id
design <- svydesign(id = ~varunit, strata = ~varstrat, weights = ~expc, data = casen)
...
# Add year and save CASEN 2024 data
casen_mun <- quintil 
glimpse(casen_mun)

# Save data 
save(casen_mun, file=paste0(data_out, "Poverty_casen_RM_2024", ".RData"))
