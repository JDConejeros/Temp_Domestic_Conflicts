# 4.1 Analytical products quadrant ----
...
# Count group crimes 
crime_count <- crimes |> 
  group_by(quadrant, zone, date, group) |> 
  summarise(crime_count = n()) |> 
  group_by(quadrant, zone, group) |> 
...
# 16781856
crime_place <- crimes |>
  group_by(quadrant, zone, date, group, place_category) |>
  summarise(place_count = n()) |>
  group_by(quadrant, zone, group) |>
  complete(place_category = places) |>
  group_by(quadrant, zone, group, place_category) |>
  mutate(
...
crime_time <- crimes |>
  group_by(quadrant, zone, date, group, time_event) |>
  summarise(time_count = n()) |>
  group_by(quadrant, zone, group) |>
  complete(time_event= times) |>
  group_by(quadrant, zone, group, time_event) |>
  mutate(
...
ndvi_dis <- ndvi_dis |> 
  dplyr::select("com_code", "com", "quad_code", "quadrant", "geometry", "geometry_id", "date", "ndvi")

# Merge district-level data
data_cov <- temp_dis |> 
  left_join(ndvi_dis, by=c("com_code", "com", "quad_code", "quadrant", "geometry", "geometry_id", "date")) |> 
...
green <- data_cov |> 
  dplyr::select(quadrant, date, ndvi) |>
  group_by(quadrant) |>
  mutate(
...
data_cov <- data_cov |> 
  dplyr::select(-ndvi) |>
  left_join(green, by = c("quadrant", "date"))

glimpse(data_cov)

## Save district-level analytical data ----
save(crime_count, file=paste0(data_out, "analytical_data/", "quadrant/", "crime_dis_RM_2017_2025", ".RData"))
save(crime_place, file=paste0(data_out, "analytical_data/", "quadrant/", "crime_place_dis_RM_2017_2025", ".RData"))
save(crime_time, file=paste0(data_out, "analytical_data/", "quadrant/", "crime_time_dis_RM_2017_2025", ".RData"))
